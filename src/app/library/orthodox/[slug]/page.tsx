...
export function generateStaticParams() {
  return orthodoxStudyLibrary.map((entry) => ({ slug: entry.slug }));
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const desc = ((entry.summary ?? `Study ${entry.title} in the Orthodox library.`).slice(0, 155));
  return buildMeta({
    title: entry.title,
    description: desc,
    keywords: `${entry.title}, Eastern Orthodox, Orthodox theology, Orthodox bible study, Septuagint`,
    path: `/library/orthodox/${slug}`,
  });
}
